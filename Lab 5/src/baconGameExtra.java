...
public class baconGameExtra<V, E> {
	...
	public static <V, E> int baconNumber(DirectedAdjListMap<V, E> directedGraph, AdjacencyListGraphMap<V,E> undirectedGraph, V name, boolean print){
	...
	public static void main(String[] args) throws Exception {
		...
		Reader actorFile = new FileReader("actors.txt");
    	Reader movieFile = new FileReader("movies.txt");
    	Map<String, String> actorMap = createIDMap(actorFile);
    	Map<String, String> movieMap = createIDMap(movieFile);
    	
    	Reader actorMovieFile = new FileReader("movie-actors.txt");
    	...
    	Map<String, Integer> memoizedActors = new HashMap<String, Integer>();
		...
		for(Vertex<String> vertex : directedActorMovieGraph.vertices()) {
			int temp = baconNumber(directedActorMovieGraph, movieActorGraph, vertex.element(), false);
			if (temp > largestPathNum) {
				largestPathNum = temp;
				actor = vertex;
			}
		}
		System.out.println(actor.element() + " has largest bacon numer of " + largestPathNum + ".");
    	...
    	while (!search.equals("q")){ //as long as the user does not quit
    		...
    		if (!movieActorGraph.vertexInGraph(search)){ //if the given actor name is not a vertex in undirected graph
    		...
    		if (memoizedActors.containsKey(search)) {
    			num = memoizedActors.get(search);
