...
public class Sketch {
	...
	public synchronized int container(int x, int y) {
		...
		int temp = -1; // creates a temporary variable that holds an index of -1 (not selected)
		
		for (int idx = shapes.size() - 1; idx >= 0; idx--) {
			if (shapes.get(idx) != null && shapes.get(idx).contains(x, y)) { //finds shape that contains the point
				temp = idx;
				return temp;
			}
		}
