...
public class HeaderSll<T> implements CS10LinkedList<T> {
    ...
    // Instance variables
    private Element<T> currentPred;// position before current in the list
    ...
    public T getFirst() {
        ...
        currentPred.next = head.next; //value after head is first element
        return get();
    }
    ...
    public T getLast() {
        if (isEmpty()) {
        	...
        	while (currentPred.next.next != null) { //iterates through list, when value after current is null,
        		currentPred = currentPred.next; //sets the last element
            }
    ...
    public void addFirst(T obj) {
        ...
        currentPred = head; //changes the currnet
    }
