...
/**
 * UniverseEC for PS-1 Catch game.
...
public class UniverseEC {
    ...
    private ArrayList<FlierEC> fliers;                        // all of the fliers

    /**
     * New universe with a background image and an empty list of fliers
     * @param image
     */
    public UniverseEC(BufferedImage image) {
        this.image = image;		
        fliers = new ArrayList<FlierEC>();
    }
     ...
     * @param f
     */
    public void addFlierEC(FlierEC f) {
        fliers.add(f);
    }
     ...
     */
    public boolean moveFlierECs() {
    	for (FlierEC flier : fliers) {
        	flier.move(); //moves the flier
        	
        	//checks whether flier touches region or border
        	if (flier.checkWin()) {
        		wins ++;
        		System.out.println("You win!  wins: " + wins + ", losses " + losses);
        		return true;
        	}
        	if (flier.checkLose()) {
        		losses ++;
        		System.out.println("You lose! wins: " + wins + ", losses " + losses);
        		return false;
        	}
    	}
		return false;
     ...
     * Draw the fliers
     */
    public void drawFlierECs(Graphics g) {
    	//goes through fliers ArrayList and draws each flier
        for (FlierEC flier : fliers) {
        	flier.draw(g); 
        }
    }
